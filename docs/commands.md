...
Chat and sender filters honor unambiguous JID/LID links from the archived WhatsApp contacts, so either verified identifier finds the combined history after relogin. The importer accepts bare numeric values in the native contact LID field as well as full `@lid` values. Query arguments still require full JIDs; an unqualified number does not identify its namespace. Conflicting or missing links retain exact-identifier filtering. A merge refuses a different nonempty LID for an already-bound contact rather than discarding the prior identity evidence; an empty incoming LID retains the last known archived value but does not authorize new cross-store matches.
